/**
 * Organism: StatsCards — matches the reference design with neon-bordered cards.
 */
import { Monitor, AlertTriangle, Users, User, Package } from 'lucide-react';
...
export default function StatsCards({ stats }) {
    ...
    const values = {
        total_computers: stats.total_computers ?? 0,
        stock_computers: stats.stock_computers ?? 0,
        assigned_computers: stats.assigned_computers ?? 0,
        faulty_computers: stats.faulty_computers ?? 0,
        total_employees: stats.total_employees ?? 0,
    };

    return (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 16 }}>
            {cards.map(({ key, label, icon: Icon, color, bg, border, glow, field, sub }) => (
                <div
                    key={key}
                    style={{
                        position: 'relative',
                ...
                >
                    ...
                    <p style={{
                        ...
                        {values[field].toLocaleString('tr-TR')}
                    </p>
